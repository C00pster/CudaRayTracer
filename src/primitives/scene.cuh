...
// __host__
// void create_scene(World** d_world, Camera** d_camera, DeviceImage* d_img, curandState *rand_state, int X, int Y, int scene) {
//     int num_primitives;
//     Primitive **d_list;
...
//             init_device_image<<<1, 1>>>(d_img, h_device_image, width, height, channels);
//             cudaDeviceSynchronize();

//             globe_device<<<1, 1>>>(d_list, d_sorted_list, d_img, d_world, morton_codes, sorted_morton_codes, d_camera, X, Y, rand_state);
//             break;
//         }
//         default:
//             std::cerr << "Invalid scene number" << std::endl;
